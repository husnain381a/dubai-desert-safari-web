...
function PackageDetail() {
  ...
  if (!p)
    return (
      <SiteLayout>
        <div className="pt-32 mx-auto max-w-7xl px-4 py-20">Loading…</div>
      </SiteLayout>
    );
          ...
          <div className="mt-6 grid gap-10 lg:grid-cols-5">
                  ...
                  <ul className="mt-4 grid sm:grid-cols-2 gap-2">
                    {(p.highlights as string[]).map((h) => (
                      <li key={h} className="flex gap-2">
                        <Check className="h-4 w-4 text-primary mt-1 shrink-0" /> {h}
                      </li>
                    ))}
                  </ul>
                </>
            ...
            <aside className="lg:col-span-2">
              <div className="sticky top-28 rounded-2xl bg-card border border-border p-6 shadow-lg">
                ...
                <div className="mt-1 text-lg text-muted-foreground line-through">
                ...
                <a
                  href={waLink(`Hi, I'd like info on ${p.title}`)}
                  target="_blank"
                ...
                <ul className="mt-6 space-y-2 text-sm text-muted-foreground">
                  <li className="flex gap-2">
                    <Check className="h-4 w-4 text-primary mt-0.5" /> Free cancellation up to 24h
                  </li>
                  <li className="flex gap-2">
                    <Check className="h-4 w-4 text-primary mt-0.5" /> Instant confirmation
                  </li>
                  <li className="flex gap-2">
                    <Check className="h-4 w-4 text-primary mt-0.5" /> Pickup from your hotel
                  </li>
                </ul>
              </div>
            </aside>
