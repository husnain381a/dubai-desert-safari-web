  ...
  head: () => ({
    meta: [
      { title: "Packages — Red Sand Dunes DXB" },
      {
        name: "description",
        content:
          "Browse and book all our desert safari and tour packages: evening, morning, overnight, VIP and more.",
      },
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((c) => (
                <button
                  key={c}
                  onClick={() => setCat(c)}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-colors capitalize ${cat === c ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-accent"}`}
                >
                  {c.replace("-", " ")}
                </button>
              ))}
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((p) => (
              <article
                key={p.id}
                className="group overflow-hidden rounded-2xl bg-card border border-border shadow-md hover-lift"
              >
                <Link
                  to="/packages/$slug"
                  params={{ slug: p.slug }}
